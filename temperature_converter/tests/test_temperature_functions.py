...
def test_convert_celcius_to_kelvin(temperatureC, temperatureK):
    assert convert_celcius_to_kelvin(temperatureC) == temperatureK

@pytest.mark.parametrize("temperatureC, temperatureF", [(0, 32.0), (-5, 23.0), (10, 50), (5, 41.0)])
def test_convert_celcius_to_fahrenheit(temperatureC, temperatureF):
    assert convert_celcius_to_fahrenheit(temperatureC) == temperatureF

@pytest.mark.parametrize("temperatureK, temperatureC", [(0, -273.5), (3.5, -270.0), (10, -263.5), (273.5, 0.0)])
def test_convert_kelvin_to_celcius(temperatureK, temperatureC):
    assert convert_kelvin_to_celcius(temperatureK) == temperatureC
...
def test_convert_fahrenheit_to_celcius(temperatureF, temperatureC):
    assert convert_fahrenheit_to_celcius(temperatureF) == temperatureC
